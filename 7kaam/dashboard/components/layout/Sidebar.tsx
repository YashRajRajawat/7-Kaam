...
export function Sidebar({ open = false, onClose }: { open?: boolean; onClose?: () => void }) {
  ...
  return (
    <>
      {/* Scrim — only rendered on small screens, where the sidebar is a drawer. */}
      {open && (
        <div
      ...
      <aside
        className={cn(
          'fixed left-0 top-0 h-full w-[280px] bg-[#1e232a] text-white flex flex-col py-6',
          'border-r border-slate-800 z-50 shadow-md transition-transform duration-200',
      ...
      >
        ...
        <div className="px-6 mb-6 flex items-center gap-3">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-[#94a3b8] hover:text-white hover:bg-white/10 transition-colors lg:hidden"
